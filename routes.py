...
def inject_unread_notifications():
    ...
    if current_user.is_authenticated:
        ...
        unread_count = (Notification.query.filter_by(user_id=current_user.id,
                                                     read=False).count())
        logger.info(
            f"Unread notifications count for user {current_user.id}: {unread_count}"
        )
...
def community_dreams():
    ...
    public_dreams = Dream.query.filter_by(is_public=True).order_by(
        Dream.date.desc()).all()
...
def dream_patterns():
    ...
    patterns = analyze_dream_patterns(
        dreams, is_premium=(current_user.subscription_type == 'premium'))
...
def dream_new():
    ...
    if request.method == 'POST':
        ...
        sleep_quality = int(request.form.get(
            'sleep_quality')) if request.form.get('sleep_quality') else None
        ...
        try:
            if current_user.subscription_type == 'premium' or current_user.monthly_ai_analysis_count < 3:
                ...
                analysis, sentiment_info = analyze_dream(content,
                                                         is_premium=is_premium)
                dream.ai_analysis = analysis

                if sentiment_info:
                    dream.sentiment_score = sentiment_info['sentiment_score']
                    dream.sentiment_magnitude = sentiment_info[
                        'sentiment_magnitude']
                    dream.dominant_emotions = sentiment_info[
                        'dominant_emotions']
...
@app.route('/reanalize/<int:dream_id>', methods=['POST'])
@login_required
def reanalyze_dream(dream_id):
    """Reanalyze a dream and update its sentiment analysis."""
    dream = Dream.query.get_or_404(dream_id)
    if current_user.subscription_type == 'premium' or current_user.monthly_ai_analysis_count < 3:
        is_premium = current_user.subscription_type == 'premium'

        analysis, sentiment_info = analyze_dream(dream.content,
                                                 is_premium=is_premium)
        dream.ai_analysis = analysis
    ...
    db.session.add(dream)
    db.session.commit()
    flash('Dream reanalyzed successfully!')
    return redirect(url_for('dream_view', dream_id=dream.id))
...
def dream_view(dream_id):
    ...
    return render_template('dream_view.html',
                           dream=dream,
...
def add_comment(dream_id):
    ...
    try:
        ...
        logger.info(
            f"Created comment with ID {comment.id} by user {current_user.id}")
        ...
        if dream.user_id != current_user.id:
            ...
            notification = Notification(user_id=dream.user_id,
            ...
            logger.info(
                f"Created dream owner notification: {notification_title} for user {dream.user_id}"
            )
            ...
            if parent_id:
                ...
                if parent_comment and parent_comment.user_id != current_user.id:
                    reply_notification = Notification(
                        ...
                        content=
                        f"{current_user.username} replied to your comment: {content[:100]}{'...' if len(content) > 100 else ''}",
                        type='reply',
                        reference_id=dream_id,
                        created_at=datetime.utcnow())
                    db.session.add(reply_notification)
                    logger.info(
                        f"Created reply notification for user {parent_comment.user_id}"
                    )
...
@app.route('/dream/<int:dream_id>/comment/<int:comment_id>/edit',
           methods=['POST'])
@login_required
...
def create_forum_post(group_id):
    ...
    membership = GroupMembership.query.filter_by(user_id=current_user.id,
    ...
    if request.method == 'POST':
        try:
            post = ForumPost(title=request.form.get('title'),
                             content=request.form.get('content'),
                             user_id=current_user.id,
                             group_id=group_id)
...
def reply_to_post(post_id):
    ...
    membership = GroupMembership.query.filter_by(
        user_id=current_user.id, group_id=post.group_id).first()
    ...
    try:
        reply = ForumReply(content=content,
                           user_id=current_user.id,
                           post_id=post_id)
    ...
    try:
        if action == 'hide':
            ...
            notification = Notification(
                ...
                content=
                f"Your comment has been hidden by a moderator. Reason: {reason}",
                type='moderation',
            ...
            if comment.parent_id is None:
                for reply in comment.replies:
                    if not reply.is_hidden:
                        reply.hide(current_user,
                                   f"Parent comment hidden: {reason}")
                        reply_notification = Notification(
                            user_id=reply.user_id,
                            title="Your reply has been hidden",
                            content=
                            f"Your reply has been hidden because the parent comment was hidden. Reason: {reason}",
                            type='moderation',
                            reference_id=comment.dream_id)
                        db.session.add(reply_notification)
            ...
            if comment.parent_id is None:
                for reply in comment.replies:
                    if reply.is_hidden and reply.moderation_reason and reply.moderation_reason.startswith(
                            "Parent comment hidden:"):
                        ...
                        reply_notification = Notification(
                            ...
                            content=
                            "Your reply has been restored because the parent comment was restored.",
                            type='moderation',
                            reference_id=comment.dream_id)
                        db.session.add(reply_notification)
...
def toggle_moderator(user_id):
    ...
    try:
        ...
        db.session.commit()
        flash(
            f'Moderator status {"granted to" if user.is_moderator else "removed from"} {user.username}.'
        )
    except Exception as e:
