...
def create_groups(users):
    ...
    groups = []
    group_templates = [
        ("Lucid Dreams Explorers",
         "A group dedicated to exploring and understanding lucid dreams."),
        ("Nightmare Support",
         "A safe space to discuss and cope with nightmares."),
        ("Dream Interpreters",
         "Share and interpret the symbolism in your dreams."),
        ("Flying Dreams", "For those who experience flying in their dreams."),
        ("Prophetic Dreams",
         "Discuss potentially prophetic or precognitive dreams."),
        ("Nature Dreams", "Share dreams involving nature and wildlife.")
    ]

    for name, description in group_templates:
        creator = random.choice(users)
        group = DreamGroup(name=name,
...
def create_memberships(users, groups):
    ...
    for group in groups:
        ...
        creator_membership = GroupMembership(user_id=group.created_by,
                                             group_id=group.id,
                                             is_admin=True,
...
def seed_database():
    ...
    try:
        ...
        print("Dreams created successfully.")
